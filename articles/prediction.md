...
plot(seq_along(testY_subset), testY_subset,
...
points(seq_along(testY_subset), mean_preds, pch = 4, col = "blue", cex = 1)

# Add credible intervals (narrower, in blue)
for (i in seq_along(testY_subset)) {
...
legend("topright",
  legend = c(
    "Observed Values", "Mean Prediction",
