...
test_that("AddiVortes fit 1 with quantile check", {
  ...
  results <- AddiVortes(Y,X,10,
                        ...
                        Y_test,X_test,
                        IntialSigma = "Linear")
  predictions <- PredictAddiVortes(results[[1]],
                                   X_test,
                                   Y_test,
                                   c(0.5,0.75))

  # Check ordering of all predictions
  expect_true(all(predictions[[3]][1,] <= predictions[[3]][2,]))
  expect_equal(round(as.numeric(predictions[[3]][2,1]),3),
               0.380)
  expect_equal(round(as.numeric(predictions[[3]][1,10]),3),
               -0.098)
})
...
test_that("AddiVortes fit 3 with quantile count", {
  ...
  expect_equal(length(predictions[[3]][,1]), 7)
})
